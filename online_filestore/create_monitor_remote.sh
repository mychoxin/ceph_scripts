...
function usage()
{
	echo "Usage:$0 -t|--target <monitor node ip> [-a|--add] [-u|--unformat] [-h|--help]"
	echo "-t, --target <monitor node ip>"
	echo -e "\tan ip used to ssh and remote create monitor, make sure SSH login without passwd, not support ipv6 or lookback interface"

...
function usage()
{
	echo "Usage:$0 -t|--target <osd node ip> [-d|--device <osds device>] [-h|--help]"
        echo "-t, --target <osd node ip>"
        echo -e "\tan ip used to ssh and remote delete osd, make sure SSH login without passwd, not support ipv6 or lookback interface"

        echo "[-d, --device <osd disk>]"
        echo -e "\tosd data disk, not a partition, but the whole disk"
        echo -e "\tand we will delete the osds in these disks"
        echo -e "\tif not given, then all the osds in the machine will be removed."

...
function usage()
{
	...
	echo "[[ip1] [ip2]...]"
	echo -e "\tIPs of all nodes, if not given than use host in ceph.conf, use space seperate"

        echo "[-n, --noninteractive]"
        echo -e "\tforce to destory the cluster without asking the user"

        echo "[-f, --force]"
        echo -e "\tforce to delete all of the searched device partitions even though they are not used by the cluster"

...
function usage()
{
        echo "Usage:$0 -t|--target <osd node ip> [-u|--unformat] " \
	...
	" [-h|--help]"
        ...
        echo -e "\tan ip used to ssh and remote create osd, make sure SSH login without passwd, not support ipv6 or lookback interface"
...
if ! temp=$(getopt -o n:r:t:uh --long osd-num:,rule-num:,target:,unformat,help -n 'note' -- "$@" 2>&1)
then
	parse_failed "$temp"
        exit 1
fi
...
function parse_and_check_params()
{
	...
	add_log "INFO" "osd_num_in_each_journal=$data_disks_of_each_journal" $print_log
	check_datadisks_num_of_journal "$data_disks_of_each_journal" || return 1
	local_opt="-r $data_disks_of_each_journal $local_opt"
...
function create_osd()
{
	...
	if ! $SSH ${user}@$remote_ip "$remote_tmp_dir/create_osds_local.sh $local_opt" 2>&1
	then

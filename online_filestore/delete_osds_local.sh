...
function parse_and_check_params()
{
	...
	for((i=0; i<$data_disks_total_count; ++i))
		...
		mount_devs=($(mount |grep ^"$dev"|awk '{print $1}'))
		osd_ids=()

		add_log "INFO" "mount-points=${mount_point[*]}" $print_log
		add_log "INFO" "mount-devs=${mount_devs[*]}" $print_log

		for((j=0; j<${#mount_point[@]}; ++j))
		do
			local osd=$(awk -F" = " -v dir="${mount_point[$j]}" '/\[osd\..*\]/{a=1;osd_line=NR;find=$0}a==1&&$2==dir{print find":"osd_line":"NR}' $ceph_conf)
			osd_ids[$k]="$osd"
			((k++))
		done
	done
...
function remove_osds()
{
	...
	do
		...
		remove_one_osd $num $remove_wait
                ...
                local osd_data="${osd_data_dir}/osd-${osd_id}"
		if [ -d "$osd_data" ]
		then
